 ...
 * sits at the root of the archive. Chrome and Firefox both expect that.
 *
 * Output: extension/purgeq-<version>.zip
 *
 * Pure stdlib so no extra dependency is required.
 */
...
// CRC-32 / IEEE 802.3 — needed for the ZIP central directory. Inlined so
// we don't depend on zlib.crc32 (only available since Node 22).
const crcTable = (() => {
...
// Tiny ZIP writer. Only supports stored or deflated regular files, which is
// all we need for an extension package.
...
for await (const { full, rel } of walk(distDir)) {
...
console.log(`✓ Packaged ${localHeaders.length} files → ${path.basename(out)}`);
