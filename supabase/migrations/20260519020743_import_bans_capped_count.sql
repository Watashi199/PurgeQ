-- Split import_bans return type: add `capped` column to distinguish rows
-- blocked by the free tier cap from true duplicates/invalid rows (skipped).
drop function if exists public.import_bans(jsonb);

create function public.import_bans(p_rows jsonb)
returns table(imported int, skipped int, capped int)
...
begin
  ...
  if p_rows is null or jsonb_typeof(p_rows) <> 'array' then
    raise exception 'p_rows must be a JSON array' using errcode = 'P0001'; end if;
  if jsonb_array_length(p_rows) > MAX_ROWS then
    raise exception 'Too many rows (% max)', MAX_ROWS using errcode = 'P0008'; end if;
  select id into v_banlist_id from banlists where owner_id = v_user_id;
  if v_banlist_id is null then raise exception 'No banlist owned by user' using errcode = 'P0001'; end if;
  select coalesce(is_pro, false) into v_is_pro from profiles where id = v_user_id;
  if not v_is_pro then select count(*) into v_ban_count from bans where banlist_id = v_banlist_id; end if;
  for v_row in select * from jsonb_array_elements(p_rows) loop
    if not v_is_pro and v_ban_count >= FREE_BAN_LIMIT then v_capped := v_capped + 1; continue; end if;
    ...
    if v_faceit_name = '' or length(v_faceit_name) < 2 or length(v_faceit_name) > 32
       ...
       or v_author_name = '' or length(v_author_name) > 64 then
  ...
  return query select v_imported, v_skipped, v_capped;
end;
$$;
